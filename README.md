...
- Supports action controls rendered as links/buttons **or** input submit buttons (`value="Enter Date"`).
- Supports `See Details` action labels for flows where reserve starts from details page.
- Matches site codes using common variants (e.g., `L2`, `L 2`, `L-2`, `Loop L Site 2`).
- Includes a row-proximity heuristic to click `Enter Date` when site labels/actions are siblings.
