...
async function configureTripDates(page) {
  ...
  const departure = cfg.endDate || addDaysIso(cfg.startDate, cfg.nights);
  if (!departure) {
    log(`Date config ignored: invalid START_DATE/END_DATE values (START_DATE=${cfg.startDate}, END_DATE=${cfg.endDate})`);
    return;
  }
